...
let myAge = 42;
let dogAge = 6;
console.log("My dog is older:", dogAge * 7 > myAge);
myAge++;
dogAge++;
console.log("My dog is older next year:", dogAge * 7 > myAge);
```
...
let getsExtraCookie = orderedCake && (orderedTea || orderedCoffee);
```
...
Solution: First condition is `true`, so it prints "you are allowed to drive a car"
<!-- .element: class="fragment" -->
...
The same example, but now with `if..else`:
...
* Write three if statements
  ...
  * Make a statement that prints "I am younger" if your age is less than your neighbor's, otherwise "I am not younger"
